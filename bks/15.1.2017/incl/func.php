<?php
	...
	function dbresultone($q){ // do one db select query to get results
		global $link;
	...
	function jumboHead($title,$jid = '') { // for jumbotran head
		...
		$con .= '<h3 class="text-center headimageText">'.$title.'</h3> ';
		$con .= '</div>';
	...
	function getusertype($userlogid) {
		global $link;
	...
	function urlredirect($pathfrom,$pathto) { // redirects to another url, if wrong url typed
		if($_SERVER['REQUEST_URI'] == $pathfrom) {
			redirect($pathto);
		}
	
	}
	
	function bsalert($alertType,$message,$alertMain="",$alertflag="p") { // alert success or error E - for err and S- for success
		if($alertType == 'S') {
			$alertHead = "success";
			if($alertflag!="p") {
				$alertMain = "Success !";
