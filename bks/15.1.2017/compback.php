...
<?php
	...
	if(isset($_POST['csubBut'])) {
		...
		if($emptyCheck) {
			...
			if($result) {
				//echo "Success";
				//$alertType,$message,$alertMain="",$alertflag="p") { // alert success or error E - for err and S- for success
				echo bsalert('S','Complaint created','','w');
			} else {
				echo bsalert('E','Unable to rasie complaint. Retry.');
